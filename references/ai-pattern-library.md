...
- Using dashes anywhere in the article body: bullet dashes (`- item`), mid-sentence dashes (`X - Y`), or em dashes (`X — Y`) — rewrite as separate sentences, use commas/colons, or use numbered lists instead

## Phrase-Level Red Flags

Scan the draft and flag any of the following. Replace each with human alternatives:

| AI Pattern | Human Alternative |
|---|---|
| "It's important to note that..." | Just state the thing directly |
| "In the realm of..." | "In [specific field]..." or cut it |
...
- Headings that use `— ` or ` - ` as a separator → rewrite as single natural sentence or use `:` sparingly
...
2. Rewrite flagged sections using alternatives from the tables above.
